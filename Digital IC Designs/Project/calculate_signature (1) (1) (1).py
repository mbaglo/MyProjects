...
sisrtaps = np.array(
    [10, 12, 13, 15]
)  # SISR tap locations: x^16 + x^14 + x^13 + x^11 + 1 (positions 0, 13, 12, 10)
...
for step in range(2**sisrn - 1):
    ...
    if sisrstyle == 0:
        ...
        # Feedback bit is the LSB (position 0)
        feedback = sisr[-1] if 0 in sisrtaps else 0
        # Apply feedback to positions excluding the feedback bit
        tap_mask = np.array(
            [int(np.isin(x, [t for t in sisrtaps if t != 0])) for x in range(sisrn - 1)]
        )
        ...
        sisr = np.concatenate(
            (
                ...
                (
                    ...
                    * np.array(
                        [
                            int(np.isin(x, [t for t in sisrtaps if t != 0]))
