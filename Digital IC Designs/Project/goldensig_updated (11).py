...
for step in range(nsteps):
    ...
    # Mapping: A0-A7 (LSB to MSB) from PRSG bits 0-7 (prsg[8:16]), B0-B7 (LSB to MSB) from PRSG bits 8-15 (prsg[0:8])
    in1 = 0  # A0-A7 (PRSG bits 0-7, prsg[8:16])
    for ele in prsg[8:16]:
        in1 = (in1 << 1) | int(ele)

    in2 = 0  # B0-B7 (PRSG bits 8-15, prsg[0:8])
    for ele in prsg[0:8]:
        in2 = (in2 << 1) | int(ele)
    ...
    # Input to SISR (Galois configuration, shift right D0 to D15)
    if sisrstyle == 0:
        feedback = np.sum(sisr[sisrtaps]) % 2
        feedback = (feedback + sisr[0]) % 2
        new_bit = (single_bit_output + feedback) % 2
        sisr = np.concatenate(([new_bit], sisr[:-1]))
    else:
        # Galois SISR: apply feedback before shift, then insert new bit at D0
        feedback = sisr[0]  # D15
        # Apply feedback at tap positions (15, 13, 12, 10)
        new_sisr = sisr.copy()
        ...
        # Shift right and insert new bit at D0
        new_sisr = np.concatenate(([single_bit_output], new_sisr[:-1]))
        sisr = new_sisr
